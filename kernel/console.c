...
int
cread3(int user_dst, uint64 dst, int n)
{
  return consoleread(&cons3, user_dst, dst, n);
}

int
cwrite3(int user_src, uint64 src, int n)
{
  return consolewrite(&cons3, user_src, src, n);
}
...
consoleinit(void)
{
  ...
  devsw[CONSOLE3].read = cread3;
  devsw[CONSOLE3].write = cwrite3;
}
